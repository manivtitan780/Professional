...
public class DocumentTypeValidator : AbstractValidator<DocumentType>
{
    ...
    public DocumentTypeValidator()
    {
        ...
        RuleFor(x => x.DocType).NotEmpty().WithMessage("Document Type is required.")
                               .Length(2, 50).WithMessage("Document Type should be between {MinLength} and {MaxLength} characters.")
                               .Must((obj, docType) => CheckDocumentTypeExists(docType, obj.ID)).WithMessage("Document Type already exists. Enter another Document Type.");
    }
