...
// Created On:          03-02-2023 19:11
// Last Updated On:     09-27-2023 15:59
// *****************************************/
...
public partial class ViewWordDocument
{
    ...
    private List<object> _items = new()
                                  {
                                      ...
                                      {
                                          Id = "save", Text = "Save", PrefixIcon = "savePrefix"
                                      },
                                      new CustomToolbarItemModel
                                      {
                                          Id = "print", Text = "Print", PrefixIcon = "printPrefix"
                                      },
                                      "Find",
                                      new CustomToolbarItemModel
                                      {
                                          Id = "close", Text = "Close", PrefixIcon = "closePrefix"
                                      }
                                  };
    ...
    private async Task OpenDialog()
    {
        ...
        MemoryStream _stream = new(await File.ReadAllBytesAsync(Path.Combine(Start.UploadsPath, "Uploads", _entity, EntityID.ToString(), InternalFileName)));
        //PdfViewer.LoadAsync("data:application/pdf;base64," + _base64String);
        WordDocument _document = WordDocument.Load(_stream, ImportFormatType.Doc);
        SerializedString = JsonSerializer.Serialize(_document);
        SfDocumentEditor _editor = EditorDocument.DocumentEditor;
        _editor?.OpenAsync(SerializedString);
    ...
    /// </remarks>
    private void Opening(BeforeOpenEventArgs obj)
    {
        IsOpening = true;
    }
    ...
    /// </remarks>
    private async Task OverlayClick(OverlayModalClickEventArgs args)
    {
